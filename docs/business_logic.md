...
- Để UI không bị chờ vô hạn với cơ cấu nhiều cont/nhiều mã, CP-SAT có giới hạn thời gian riêng cho mode này. Trạng thái `OPTIMAL` nghĩa là đã chứng minh tối ưu; `FEASIBLE` nghĩa là đã tìm được phương án đáp ứng đủ cont nhưng chưa chứng minh hết các mục tiêu phụ; nếu phải rơi về beam search thì đánh dấu `APPROXIMATE`.
