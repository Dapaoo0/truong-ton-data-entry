...
- Objective của mode này: tối thiểu `active_bunches_estimated` → giảm số đoạn cắt → giảm kg phân bổ → giảm kg dư do làm tròn → tie-break dải nải ổn định.
- Để UI không bị chờ vô hạn với cơ cấu nhiều cont/nhiều mã, mode này dùng solver MIP exact với giới hạn thời gian chứng minh. Chỉ trạng thái `OPTIMAL` mới được xem là kết quả hợp lệ. Nếu solver chỉ tìm được phương án khả thi nhưng chưa chứng minh tối ưu, hệ thống không hiển thị số buồng xấp xỉ mà báo chưa chứng minh được tối ưu trong giới hạn hiện tại.
