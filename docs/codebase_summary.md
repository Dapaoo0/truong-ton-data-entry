...
- **`app.py` (Dashboard cốt lõi)**: 
  ...
  - `render_container_allocation_calculator()`: Page Kinh doanh "Máy tính phân bổ cont". Có 2 mode: (1) Theo đơn hàng, lấy nguồn buồng từ dự báo cắt bắp hoặc nhập tay, ưu tiên theo khách hàng `Wismettac (Nhật 1)`, `Advance (Nhật 2)`, `Uone`; (2) Tối đa cont theo thị trường. Kết quả trung tâm là `Buồng xẻ tối thiểu`, tức số buồng nguyên ít nhất cần mở để đáp ứng đơn hàng/cont theo ưu tiên.
...
- **`container_allocation.py` (Optimizer phân bổ container)**:
  ...
  - `allocate_bunches_optimized()` dùng OR-Tools CP-SAT khi khả dụng. Objective lexicographic: giảm thiếu thùng theo `customer_priority` khi có, fallback `market_priority` → giảm `active_bunches_estimated` (số buồng nguyên cần xẻ) → giảm số segment → giảm nải-buồng → giảm kg dư. Một dòng đơn hàng có thể bẻ thành nhiều khoảng con nếu điều đó cải thiện thiếu hàng hoặc giảm buồng xẻ, nhưng segment penalty ngăn bẻ vụn không cần thiết.
