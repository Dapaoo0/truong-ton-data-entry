...
### 0. Lớp Nghiệp Vụ Dùng Chung (`domain/`)
- **`domain/lifecycle.py`**: Tách các helper thuần về vòng đời lô/đợt khỏi `app.py`, gồm tỉ lệ hao hụt theo giai đoạn, kg/buồng theo vụ, chuẩn hóa `base_lot_id`, map boundary vụ kế tiếp và label đợt trồng. `app.py` giữ wrapper mỏng để không phá call site Streamlit hiện tại.
- **`domain/forecast.py`**: Tách logic gom dự báo thu hoạch từ dữ liệu Cắt bắp theo tuần ISO, bao gồm lựa chọn `+8/+9` tuần inclusive và micro-spread ±7 ngày. Đây là lớp sẽ được FastAPI/worker tái sử dụng khi chuyển dần khỏi Streamlit.
- Mục tiêu: Streamlit, backend API tương lai và worker/report dùng cùng một logic nghiệp vụ, tránh copy rule giữa UI web/mobile/API.
...
- **`app.py` (Dashboard cốt lõi)**: 
  ...
  - `destruction_documents.py`: Xác thực PDF (extension, MIME, header `%PDF-`, 10 MB), tạo Storage path, upload vào bucket private, tạo metadata, bulk insert các dòng Xuất hủy dùng chung `document_id`, rollback file/metadata khi lỗi và tạo signed URL 10 phút để xem lại. Client này dùng riêng `SUPABASE_SERVICE_ROLE_KEY`; client anon hiện tại không được cấp quyền Storage.
  ...
  - `render_cost_dashboard()` (`cost_dashboard.py`): Page/tab read-only `Chi phí & định mức` dùng chung cho mọi account. Page có 2 tab nội bộ: `Chi phí` đọc raw `fact_nhat_ky_san_xuat.thanh_tien` + `fact_vat_tu.thanh_tien`; `Định mức` đọc các dòng nhật ký công có `dinh_muc > 0`, `klcv`, `so_cong > 0` để phân tích tỷ lệ hoàn thành công việc. Cả hai dùng Supabase REST client hiện tại, join dim bằng pandas, cache 5 phút và scope farm theo account. Admin/Kinh doanh xem được nhiều farm; account farm/đội chỉ xem farm đang đăng nhập. Đây là dashboard tổng quan raw, tách biệt với popup `chi phí/cây` đã clean theo lifecycle trên bản đồ.
  ...
  **Excel Export Functions:**
  ...
  - `size_report.py`: Module thuần cho nghiệp vụ Đo size. `validate_size_measurement_sequence()` kiểm tra Lần 1-2-3 và giữ cùng Hàng kiểm tra; `compute_size_forecast()` tính mức tăng size trung bình/cảnh báo chuẩn bị thu hoạch; `generate_size_measure_excel()` tạo báo cáo ngang theo mẫu, giữ nhóm hoàn tất tối đa 30 ngày và highlight các dòng dự kiến đạt 36/37 cal. UI tải riêng từng farm.
...
- Quản lý versioning cho Schema tại Supabase DB. Tất cả thay đổi lớn đều có file SQL backup.
  ...
  - `migration_size_measure_lan_do_3.sql` (Mở rộng constraint Đo size từ Lần 1-2 sang Lần 1-2-3).
...
### 7. Backend API read-only (`services/api/`)
- **`services/api/app/main.py`**: FastAPI entrypoint cho backend tách dần khỏi Streamlit. Hiện có `GET /health` và `GET /forecasts/cut-bud-weekly`.
- **`services/api/app/routers/forecasts.py`**: Đọc `stage_logs` giai đoạn `Cắt bắp` qua Supabase REST client, normalize thành dataframe và gọi `domain.forecast.build_weekly_cat_forecast()`. Test dùng dependency override nên không chạm DB thật.
- **`services/api/Dockerfile`**: Build API độc lập từ root repo, copy `domain/` và `services/` để triển khai sau này trên Cloud Run/Fly/Render hoặc hạ tầng tương đương.
