...
- **`app.py` (Dashboard cốt lõi)**: 
  ...
  - `allocate_fifo_quantity(farm_name, lo_name, new_sl, log_type, target_date, action_type, giai_doan)`: Phân bổ FIFO theo `ngay_trong`. Query tất cả base_lots active, tính capacity per-batch, split allocations. Trả về list `[{dim_lo_id, base_lot_id, so_luong, lot_id}]`.
  ...
  **Mô hình Dự báo Thu hoạch (Harvest Forecast — 3 Mốc):**
  ...
  - **3 Mốc Dự báo**: ① Từ Trồng (trừ xuất hủy thực tế), ② Từ Cắt bắp, ③ Thực tế Thu hoạch.
