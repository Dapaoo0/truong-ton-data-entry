...
- **`container_allocation.py` (Optimizer phân bổ container)**:
  ...
  - `calculate_min_bunches_for_container_plan()` là mode tính ngược từ cơ cấu cont sang số buồng xẻ tối thiểu; demand là hard constraint, sau đó solver giảm số buồng, số segment, kg phân bổ và kg dư. Mode này dùng timeout CP-SAT riêng ngắn hơn để tránh treo UI với cơ cấu nhiều cont/nhiều mã: `OPTIMAL` là đã chứng minh tối ưu, `FEASIBLE` là đủ demand nhưng chưa chứng minh hết tie-break, `APPROXIMATE` là fallback beam search.
