...
- **`app.py` (Dashboard cốt lõi)**: 
  ...
  **Luồng dữ liệu chính (Data Flow):**
  - `df_lots_all` ← `fetch_table_data("base_lots")` — toàn bộ đợt trồng (cả trồng mới + trồng dặm).
  - Merge `loai_trong` từ `df_seasons` (join `base_lot_id`, filter `vu='F0'`) vào `df_lots_all`.
