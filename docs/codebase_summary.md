...
- **`container_allocation.py` (Optimizer phân bổ container)**:
  ...
  **Bản đồ Farm (SVG Interactive Map):**
  ...
  - Tooltip ghim của từng lô có nút `Xem chi phí/cây`, điều hướng bằng query params `cost_farm` + `cost_lot` để mở `@st.dialog` dashboard chi phí. Dashboard cộng `fact_nhat_ky_san_xuat` + `fact_vat_tu`, giữ cả dòng gộp, và phân bổ chi phí theo từng `base_lot` trồng mới bằng tỷ lệ số cây của các đợt active tại ngày phát sinh chi phí.
