...
- **`app.py` (Dashboard cốt lõi)**: 
  ...
  - `render_container_allocation_calculator()`: Page Kinh doanh "Máy tính phân bổ cont". Có 2 mode: (1) Theo đơn hàng, lấy nguồn buồng từ dự báo cắt bắp hoặc nhập tay; (2) Tối đa cont theo thị trường. Kết quả trung tâm là `Buồng xẻ tối thiểu`, tức số buồng nguyên ít nhất cần mở để đáp ứng đơn hàng/cont theo ưu tiên.
  ...
  **Excel Export Functions:**
  - `generate_cut_bap_excel(df_lots, df_stg, df_des)`: Báo cáo Cắt bắp + Xuất hủy trước thu hoạch theo tuần, chia sheet theo năm. **4-row header**: Row 1 = Dự báo tuần thu hoạch (`_forecast_harvest_label`: `"25 (+8)/26 (+9)"`), Row 2 = Tuần X, Row 3 = CẮT BẮP/XUẤT HỦY, Row 4 = màu dây (từ `ribbon_schedule`). Cột Lũy kế cuối bảng. `data_start_row = 5`, `freeze_panes = "B5"`. Type-safe: `tuan` + `_year` cast `int`. Lot matching by `base_lot_id`.
...
- **`container_allocation.py` (Optimizer phân bổ container)**:
  - Chứa cấu hình quy cách mã hàng: `27CP Nhật 1-5`, `30CP Nhật 1-9`, `6H Nhật 5-7`, `5H Nhật 8-10`, `8H Hàn 1-4`, `5/6H Hàn 5-9`, `15CP Hàn 10-12`.
  - Các khoảng là khoảng mẹ; helper `_valid_optimizer_ranges()` sinh mọi khoảng con liền kề.
  - `allocate_bunches_optimized()` dùng OR-Tools CP-SAT khi khả dụng. Objective lexicographic: giảm thiếu thùng theo ưu tiên → giảm `active_bunches_estimated` (số buồng nguyên cần xẻ) → giảm số segment → giảm nải-buồng → giảm kg dư.
  - Hard constraint: mỗi dòng đơn hàng/mỗi mã chỉ chọn một khoảng nải liền kề duy nhất, không split cùng một mã thành nhiều đoạn rời.
