...
- **`app.py` (Dashboard cốt lõi)**: 
  ...
  **Excel Export Functions:**
  ...
  - `generate_harvest_forecast_excel(df_lots, df_stg)`: Báo cáo Dự báo Thu hoạch riêng từ dữ liệu Cắt bắp. Farm 126 dùng `+8` tuần inclusive, Farm 157 dùng `+9`; số lượng dự báo là `round(Số cắt bắp × 97%)`. Workbook có sheet `Tổng hợp` theo tuần thu hoạch dự báo và sheet `Chi tiết nguồn` để truy ngược farm/tuần cắt/màu dây/lô/base_lot. UI Admin/Phòng Kinh doanh tải một file gộp tất cả farm; account farm tải theo scope hiện tại.
