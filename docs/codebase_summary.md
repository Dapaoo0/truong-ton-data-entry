...
- **`container_allocation.py` (Optimizer phân bổ container)**:
  ...
  - `allocate_bunches_optimized()` dùng OR-Tools CP-SAT khi khả dụng. Objective lexicographic: giảm thiếu thùng theo ưu tiên → giảm `active_bunches_estimated` (số buồng nguyên cần xẻ) → giảm số segment → giảm nải-buồng → giảm kg dư. Một dòng đơn hàng có thể bẻ thành nhiều khoảng con nếu điều đó cải thiện thiếu hàng hoặc giảm buồng xẻ, nhưng segment penalty ngăn bẻ vụn không cần thiết.
