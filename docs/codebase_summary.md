...
- **`app.py` (Dashboard cốt lõi)**: 
  ...
  - `allocate_fifo_quantity(farm_name, lo_name, new_sl, log_type, target_date, action_type, giai_doan)`: Phân bổ FIFO theo `ngay_trong`. Query tất cả base_lots active, tính capacity per-batch, split allocations. Trả về list `[{dim_lo_id, base_lot_id, so_luong, lot_id}]`. **Hai chế độ**: (1) User chỉ định `base_lot_id` → trigger skip FIFO, (2) Không chỉ định → FIFO mặc định.
  ...
  **Mô hình Dự báo Thu hoạch (Harvest Forecast — 4 Mốc, Dual-Model):**
  ...
  - **Mốc ②③ (Chích/Cắt bắp — Shift-based)**: Dùng dữ liệu thực tế theo ngày từ `stage_logs`. Shift ngày chích +84d (`DAYS_CHICH_TO_THU`) / cắt +70d (`DAYS_CAT_TO_THU`). Phase xác định bằng tích lũy % so với `base_lots.so_luong`. Không trừ hao hụt ước tính.
  ...
  - **Helper `_build_shift_rows()`**: Nested function tạo shift rows cho chích/cắt bắp. Tích lũy cumulative, tách record tại ranh giới phase (while loop).
