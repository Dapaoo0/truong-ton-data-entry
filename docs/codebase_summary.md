...
- **`container_allocation.py` (Optimizer phân bổ container)**:
  ...
  - `calculate_min_bunches_for_container_plan()` là mode tính ngược từ cơ cấu cont sang số buồng xẻ tối thiểu; demand là hard constraint, sau đó solver giảm số buồng, số segment, kg phân bổ và kg dư. Mode này ưu tiên OR-Tools linear MIP (`CBC/SCIP`) để chứng minh exact; chỉ `OPTIMAL` mới được xem là kết quả hợp lệ. Nếu hết giới hạn chứng minh mà chưa tối ưu tuyệt đối, hàm trả `NO_SOLUTION` với backend `_timeout`, không trả `FEASIBLE/APPROXIMATE` để tránh hiểu nhầm số buồng là tối thiểu.
