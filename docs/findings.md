...
- **Harvest 3 Phases**: Dự báo thu hoạch dùng mô hình Normal Distribution truncated. Cửa sổ 54 ngày = Thu bói (14d, 10%) + Thu rộ (26d, 80%) + Thu vét (14d, 10%). Mỗi phase được gán vào tháng chứa midpoint của khoảng thời gian. **Tỷ lệ % có thể tùy chỉnh bởi người xem** (mặc định 10/80/10): rescale PDF weights cho mỗi phase khớp % mong muốn, giữ nguyên timeline 14/26/14 ngày cố định.
...
- **⚠️ Schema Design**: `loai_trong` (Trồng mới / Trồng dặm) hiện nằm ở `seasons.loai_trong` nhưng về bản chất thuộc về `base_lots` (thuộc tính của đợt trồng, không thay đổi giữa các vụ F0→Fn). Hiện tại code phải join `seasons WHERE vu='F0'` chỉ để lấy `loai_trong` — xem xét migrate sang `base_lots.loai_trong` trong tương lai.
