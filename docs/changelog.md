...
- **[Khách hàng]**: Mode Theo đơn hàng thêm tầng `Khách hàng` (`Wismettac (Nhật 1)`, `Advance (Nhật 2)`, `Uone`) và ưu tiên theo khách hàng trước mã hàng; kết luận vẫn gom theo thị trường vận chuyển Nhật/Hàn.
- **[Blank state]**: Bỏ default đơn hàng/ưu tiên trong mode Theo đơn hàng; người dùng phải chọn khách hàng, mã hàng và nhu cầu trước khi optimizer chạy.
- **[Mục tiêu chính]**: Mode Theo đơn hàng giờ được định nghĩa rõ là tìm `Buồng xẻ tối thiểu`: số buồng nguyên ít nhất cần mở để đáp ứng đơn hàng theo ưu tiên khách hàng/mã hàng.
