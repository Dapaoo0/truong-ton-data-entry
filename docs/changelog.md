...
#### Perf: Tăng tốc mở popup chi phí/cây (`app.py`)
- **[Luồng mở popup]**: Khi nhận event từ bản đồ, app rerun ngay và mở dialog ở đầu tab `Dữ liệu toàn cục`, trước khi fetch/render các chart và map nặng.
- **[Cache]**: Tăng TTL cache chi phí/cây lên 15 phút và tách context chi phí theo farm để các lô trong cùng farm dùng lại dữ liệu `dim_lo`, `base_lots`, `fact_nhat_ky_san_xuat`, `fact_vat_tu`, `dim_cong_viec`, `dim_vat_tu`.
- **[Loading]**: Thêm spinner `Đang tính chi phí/cây cho lô này...` trong dialog.
- **[Dismiss]**: Dấu `X`/ESC/click ngoài dialog chi phí/cây dùng `on_dismiss` để clear state giống nút `Đóng`, tránh nền trắng sau khi đóng popup.

#### Fix: Bản đồ scale đúng theo mọi kích thước màn hình (`map_template.py`)
- **[Layout]**: Tách SVG vào `.map-viewport` có `aspect-ratio` đúng theo viewBox, SVG fill tuyệt đối trong viewport; legend nằm ngoài viewport.
- **[Responsive]**: Tooltip/panel tính vị trí theo viewport bản đồ, tránh bị lệch khi màn hình PC lớn hoặc tỷ lệ màn hình khác nhau.
...
- **[Tên mode]**: Đổi 3 nhãn mode thành `Tính số hàng từ số buồng`, `Tính số cont tối đa từ số buồng`, `Tính số buồng từ số cont`.
- **[Rõ nghĩa]**: Tên mode dùng dạng "tính gì từ gì" để tránh hiểu nhầm chiều quy đổi.
- **[Tương thích]**: Giữ alias các tên mode cũ khi đọc session/kế hoạch đã lưu.

---

## [31/05/2026] - Mode Tính số buồng từ số cont

#### Feature: Tính ngược số buồng xẻ từ cơ cấu container (`container_allocation.py`, `app.py`)
- **[UI]**: Máy tính phân bổ cont có thêm mode `Tính số buồng từ số cont`; người dùng nhập từng cont riêng với khách hàng, mã hàng và số thùng.
...
- **[Tên mode]**: 3 nhãn mode hiện tại là `Tính số hàng từ số buồng`, `Tính số cont tối đa từ số buồng`, `Tính số buồng từ số cont`.
...
- **[Profile]**: Thêm profile gốc 12 nải tổng `28.4kg` và 9 nải tổng `19.0kg`; app scale theo tỷ lệ về `12 nải - 18kg`, `12 nải - 20kg`, hoặc `9 nải - 15.6kg`.
...
- **[UI]**: Máy tính cho chọn `Loại buồng`; 12 nải chọn kịch bản 18/20kg, 9 nải dùng 15.6kg. Popover hiển thị bảng kg từng nải sau quy đổi.
...
- **[Khách hàng]**: Mode `Tính số hàng từ số buồng` thêm tầng `Khách hàng` (`Wismettac (Nhật 1)`, `Advance (Nhật 2)`, `Uone`) và ưu tiên theo khách hàng trước mã hàng; kết luận vẫn gom theo thị trường vận chuyển Nhật/Hàn.
- **[Blank state]**: Bỏ default đơn hàng/ưu tiên trong mode `Tính số hàng từ số buồng`; người dùng phải chọn khách hàng, mã hàng và nhu cầu trước khi optimizer chạy.
- **[Mục tiêu chính]**: Mode `Tính số hàng từ số buồng` giờ được định nghĩa rõ là tìm `Buồng xẻ tối thiểu`: số buồng nguyên ít nhất cần mở để đáp ứng đơn hàng theo ưu tiên khách hàng/mã hàng.
...
- **[Objective refinement]**: Cho phép một dòng đơn hàng bẻ thành nhiều khoảng con khi việc đó giảm thiếu hàng hoặc giảm số buồng nguyên cần xẻ; số segment là penalty vận hành ngay sau `active_bunches_estimated` nên không bẻ vụn khi số buồng xẻ không đổi.
- **[Split penalty]**: Không còn cấm chẻ tuyệt đối; thay vào đó số segment bị phạt sau số buồng xẻ tối thiểu, nên solver chỉ chẻ khi có lợi thật.
